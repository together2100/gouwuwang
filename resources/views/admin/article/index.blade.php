       ...
       <table class="table table-striped table-bordered table-hover dataTable no-footer"
        ...
        <tbody> 
         ...
         <tr class="gradeA odd"> 
          ...
          <td class="sorting_1">{{ $row['editor']}}</td> 
          <td class="sorting_1"><img src="{{ $row['thumb'] }}" width="100px" alt=""></td> 
          <td class="sorting_1">{!!$row['descr']!!}</td> 
          <td class="sorting_1">
               <form action="/article/{{ $row['id'] }}" method="post" style="display:inline-block;">
               {{ csrf_field() }}
               {{ method_field('DELETE') }}
                    <button class="btn btn-danger" >删除</button>
                </form>
                 <a href="/article/{{ $row['id'] }}/edit" class="btn btn-info">修改</a>
          </td> 
         </tr> 
         @endforeach
        </tbody> 
